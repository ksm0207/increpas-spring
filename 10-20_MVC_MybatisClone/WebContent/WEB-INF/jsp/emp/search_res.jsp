...
<body>
		...
		<header>
			...
			<div>
				<table>
					...
					<c:if test="${search_res ne null }">
						<c:forEach var="evo" items="${search_res}">
							<tr>
								<td>${evo.employee_id}</td>
								<td><a href="view.inc?idx=${evo.employee_id}">${evo.first_name}</a></td>
								<td>${evo.last_name}</td>
								<td>${evo.job_id}</td>
	...
	<div id="search_win">
		<form method="GET" action="search.inc">
